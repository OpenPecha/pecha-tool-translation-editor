...
export function useDisplaySettings() {
	...
	useEffect(() => {
		...
		root.style.setProperty(
			"--root-editor-font-size",
			`${settings.rootEditorTypography?.fontSize}rem`,
		);
		root.style.setProperty(
			"--root-editor-line-height",
			settings.rootEditorTypography?.lineHeight.toString(),
		);

		// Translation Editor Typography
		const translationFontFamily =
			settings.translationEditorTypography?.fontFamily === "google-sans-regular"
				? "google-sans"
				: settings.translationEditorTypography?.fontFamily;
		root.style.setProperty(
			"--translation-editor-font-family",
			translationFontFamily,
		);
		root.style.setProperty(
			"--translation-editor-font-size",
			`${settings.translationEditorTypography?.fontSize}rem`,
		);
		root.style.setProperty(
			"--translation-editor-line-height",
			settings.translationEditorTypography?.lineHeight.toString(),
		);
