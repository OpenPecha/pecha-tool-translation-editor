...
export function useTranslationSettings() {
  ...
  const getInitialConfig = (): TranslationConfig => {
    try {
      ...
      console.warn(
        "Failed to load translation config from localStorage:",
        error
      );
    }
  ...
  const getInitialUIState = (): TranslationUIState => {
    try {
      ...
      console.warn(
        "Failed to load translation UI state from localStorage:",
        error
      );
    }
  ...
  const [config, setConfigState] = useState<TranslationConfig>(() =>
    getInitialConfig()
  );
  const [uiState, setUIStateState] = useState<TranslationUIState>(() =>
    getInitialUIState()
  );
  ...
  useEffect(() => {
    try {
      ...
      console.error(
        "Failed to save translation config to localStorage:",
        error
      );
    }
  }, [config]);
  ...
  useEffect(() => {
    try {
      ...
      console.error(
        "Failed to save translation UI state to localStorage:",
        error
      );
    }
  }, [uiState]);
  ...
  return {
    ...
    setIsSidebarCollapsed: (collapsed: boolean) =>
      updateUIState("isSidebarCollapsed", collapsed),
  };
}
