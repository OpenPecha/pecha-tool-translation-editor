...
const BasicCodeMirrorEditor: React.FC<BasicCodeMirrorEditorProps> = ({
  documentId,
  isEditable,
  currentDoc,
}) => {
  ...
  const debouncedTriggerSave = useDebounce((content: string) => {
    ...
    if (documentId) {
      ...
      localStorage.setItem(
        annotationsKey,
        JSON.stringify(formatRangesRef.current)
      );
    }
  }, 3000);
  ...
  const triggerSave = useCallback(
    (view: EditorView) => {
      ...
      // Store content for potential immediate save on unload
      pendingSaveContentRef.current = content;

      // Call debounced save function
      debouncedTriggerSave(content);
    },
  ...
  }, [isEditable, triggerSave]); // Only recreate when necessary
  ...
  // Force save on navigation using pending content
  useEffect(() => {
    const handleBeforeUnload = () => {
      if (hasUnsavedChanges && pendingSaveContentRef.current.trim()) {
        updateDocumentMutation.mutate(pendingSaveContentRef.current);
      }
    };
  ...
  }, [hasUnsavedChanges, updateDocumentMutation]);
...
export default BasicCodeMirrorEditor;
