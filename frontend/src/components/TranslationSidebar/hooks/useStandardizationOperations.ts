...
import { TranslationConfig, TranslationResult } from "./useTranslationOperations";
import { GlossaryTerm } from "./useGlossaryOperations";

interface UseStandardizationOperationsProps {
  config: TranslationConfig;
  getCurrentTranslationResults: () => TranslationResult[];
  updateTranslationResults: (updater: (prev: TranslationResult[]) => TranslationResult[]) => void;
...
export const useStandardizationOperations = ({
  ...
  const [isAnalyzingStandardization, setIsAnalyzingStandardization] = useState(false);
  const [standardizationStatus, setStandardizationStatus] = useState<string>("");

  // Apply standardization state
  const [isApplyingStandardization, setIsApplyingStandardization] = useState(false);
  const [applyStandardizationStatus, setApplyStandardizationStatus] = useState<string>("");
  const applyStandardizationAbortControllerRef = useRef<AbortController | null>(null);
  ...
  const startStandardizationAnalysis = async () => {
    ...
    try {
      ...
      const result = await performStandardizationAnalysis(standardizationParams);
      ...
      if (inconsistentCount > 0) {
        setStandardizationStatus(`Found ${inconsistentCount} inconsistent terms`);
      } else {
      ...
      setStandardizationStatus(`Standardization analysis failed: ${errorMessage}`);
      // Show error in UI for debugging
      setError(`Standardization analysis failed: ${errorMessage}`);
    }
  };

  const handleApplyStandardizationStreamEvent = (event: ApplyStandardizationStreamEvent) => {
    ...
    switch (event.type) {
      ...
      case "retranslation_start":
        setApplyStandardizationStatus(`Re-translating item ${event.index + 1}...`);
      ...
      case "error":
        ...
        setApplyStandardizationStatus(`Apply standardization error: ${event.message}`);
    ...
    try {
      ...
      await performStreamingApplyStandardization(
        ...
        (error: Error) => {
          ...
          setIsApplyingStandardization(false);
          setApplyStandardizationStatus(`Apply standardization error: ${error.message}`);
      ...
      setIsApplyingStandardization(false);
      const errorMessage = err instanceof Error ? err.message : "Unknown error";
      setApplyStandardizationStatus(`Apply standardization setup failed: ${errorMessage}`);
