...
export const useCopyOperations = ({
  ...
  const highlightLineWithRetry = (
    ...
    const attemptHighlight = (attempt: number) => {
      ...
      if (!success && attempt < maxRetries) {
        ...
        console.log(
          `🔄 Highlight attempt ${attempt + 1} failed for line ${lineNumber}, retrying in ${delay}ms...`
        );
        setTimeout(() => {
        ...
        console.log(
          `✅ Successfully highlighted line ${lineNumber} in translation editor on attempt ${
            attempt + 1
          }`
        );
      }
  ...
  const overwriteAllResults = (currentResults: TranslationResult[], translationResults: TranslationResult[]) => {
    const targetEditor = quillEditors.get(documentId);
    ...
    if (result.success) {
      ...
      const firstResult = translationResults.find(
        (result) => result.lineNumbers && Object.keys(result.lineNumbers).length > 0
      );
