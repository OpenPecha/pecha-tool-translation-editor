import { MAX_TEXT_LENGTH_FOR_REALTIME_COLLABORATION } from '@/utils/editorConfig';
import { LiveblocksProvider, RoomProvider } from '@liveblocks/react'
import React, { useEffect, useMemo, useState } from 'react'


function LiveBlockProvider({enabled=false,roomId,children}: {readonly enabled: boolean,readonly roomId: string,readonly children: React.ReactNode}) {
...
if (!enabled || !liveBlockKey) {
  ...
  return (
    <LiveblocksProvider publicApiKey={liveBlockKey}>
      <RoomProvider id={roomId} initialPresence={{ cursor: null }}>{children}</RoomProvider>
    </LiveblocksProvider>
  );
}
...
export const useLiveBlockActive=(currentDoc:any)=>{
  ...
  const textLength =ops.length > 0 ? ops.reduce((total, op) => {
