 ...
 */
function getArrayIndexForLogicalLine(logicalLineNumber: number, lines: string[]): number {
...
export function overwriteAllTranslations(
    ...
    if (result.lineNumbers) {
      ...
      if (lineRanges.length > 0) {
        ...
        if (!isNaN(lineNumber) && lineNumber > 0) {
          ...
          const flattenedTranslation = flattenTranslation(result.translatedText);
  ...
  if (lineTranslations.size === 0) {
    return {
      success: false,
      message: "No line number mapping found for translation results. Please try again after running translation on selected lines.",
      linesOverwritten: 0
    };
  }
  ...
  const isEditorEmpty = currentLines.length <= 1 && (currentLines[0] === '' || currentLines[0]?.trim() === '');
  ...
  lineTranslations.forEach((translatedText, logicalLineNumber) => {
    ...
    const arrayIndex = getArrayIndexForLogicalLine(logicalLineNumber, targetLines);
 ...
 */
export function validateTranslationResults(translationResults: TranslationResult[]): boolean {
  return translationResults.some(result => 
    result.lineNumbers && 
 ...
 */
export function getOverwritePreview(translationResults: TranslationResult[]): number[] {
